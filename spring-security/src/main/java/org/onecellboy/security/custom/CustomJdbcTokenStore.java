 ...
 * ***********************
 * 저장된 상태가 아닌
 * 실시간으로 Authorities 를 가져오기 위해 추가하였다.
 * ***************************
 */
public class CustomJdbcTokenStore implements TokenStore {

    private static final Log LOG = LogFactory.getLog(org.springframework.security.oauth2.provider.token.store.JdbcTokenStore.class);
    ...
    public OAuth2Authentication readAuthentication(String token) {
        ...
        try {
            ...
            {
/*
                CustomUserDetailsService customUserDetailsService = new CustomUserDetailsService(dataSource);
                UserDetails userDetails = customUserDetailsService.loadUserByUsername(authentication.getName());
                UsernamePasswordAuthenticationToken usernamePasswordAuthenticationToken = new UsernamePasswordAuthenticationToken(userDetails,authentication.getCredentials(),userDetails.getAuthorities());
    ...
    public Collection<OAuth2AccessToken> findTokensByClientId(String clientId) {
        ...
        try {
            accessTokens = jdbcTemplate.query(selectAccessTokensFromClientIdSql, new CustomJdbcTokenStore.SafeAccessTokenRowMapper(),
                    clientId);
        }
        catch (EmptyResultDataAccessException e) {
            if (LOG.isInfoEnabled()) {
    ...
    public Collection<OAuth2AccessToken> findTokensByUserName(String userName) {
        ...
        try {
            accessTokens = jdbcTemplate.query(selectAccessTokensFromUserNameSql, new CustomJdbcTokenStore.SafeAccessTokenRowMapper(),
                    userName);
        }
        catch (EmptyResultDataAccessException e) {
            if (LOG.isInfoEnabled())
    ...
    public Collection<OAuth2AccessToken> findTokensByClientIdAndUserName(String clientId, String userName) {
        ...
        try {
            accessTokens = jdbcTemplate.query(selectAccessTokensFromUserNameAndClientIdSql, new CustomJdbcTokenStore.SafeAccessTokenRowMapper(),
                    userName, clientId);
        }
        catch (EmptyResultDataAccessException e) {
            if (LOG.isInfoEnabled()) {
